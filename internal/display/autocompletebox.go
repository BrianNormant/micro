...
type PopulateFcn func(v *BufPane) (messages Messages)
...
func (a *AutocompletionBox) Open(pop PopulateFcn, acceptEnter, acceptTab AcceptFcn, v *BufPane) {
	a.Pop = pop
...
func (a *AutocompletionBox) OpenNoPrompt(pop PopulateFcn, acceptEnter, acceptTab AcceptFcn, v *BufPane) {
	a.Pop = pop
	a.generateAutocomplete(v)
	a.open = true
	a.showPrompt = false
...
func (a *AutocompletionBox) generateAutocomplete(v *BufPane) {
	a.messages = a.Pop(v)
...
func (a *AutocompletionBox) Display(v *BufPane) {
	if !a.open {
		return
	}
	h := 0
	...
	a.selected = util.Min(a.selected, len(a.messagesToshow)-1)

	skipped := util.Max(0, a.selected-5)
	messages := a.messagesToshow[skipped:]

	for i, message := range messages[:util.Min(len(messages), 11)] {
...
func (a *AutocompletionBox) HandleEvent(event tcell.Event, v *BufPane) (swallow bool) {
	switch e := event.(type) {
	case *tcell.EventKey:
		switch e.Key() {
		case tcell.KeyEnter:
